...
namespace PFB
{
  ...
  public:
    ...
    /// Set the refresh minutes for the PlaceFile. Setting 1 of these will
    /// cause the other to set zero.
    void setRefreshMinutes(const unsigned int m);
    void setRefreshSeconds(const unsigned int s);

    /// Get the viewing threshold.
    unsigned int getThreshold() const;

    /// Get the refresh time in minutes or seconds. If one returns zero, use 
    // the other! e.g if minutes are zero, the value is in seconds.
    unsigned int getRefreshMinutes() const;
    unsigned int getRefreshSeconds() const;
  ...
  private:
    ...
    unsigned int _refreshSeconds = 0;
